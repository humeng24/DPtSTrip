...
save_path=f"/data/hm/save_model/{dataset}/Euclidean/"
log_path = f"/data/hm/log/{dataset}/Euclidean/"
if(not os.path.exists(save_path)):
        os.makedirs(save_path)
if(not os.path.exists(log_path)):
        os.makedirs(log_path)
base_model_path = f"/data/hm/save_model/{dataset}/baseline/resnet_cifar10_512nodenum_0.01lr_32size_6666seed_without_normalization_SGD1.pth"
# 网络设置
if 'mnist' in dataset:
    input_cl = 1
    img_size = 28
...
#----------------改-----------------------------------------------------------
        ...
        margin = torch.tensor([2.5]).to('cuda:1')
...
if __name__ == '__main__':
    ...
    #optimizer = optim.Adam(model.parameters(), lr=lr,weight_decay=1e-4)     # 使用RMSprop优化器

    #scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[5, 10], gamma=0.1)## 学习率衰减 milestones为数组，gamma为倍数，当epoch为5时，lr变为原始的0.1倍；为10时，lr变为原始的0.01倍
    #scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=50, eta_min=1e-06)
    #scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=50, eta_min=1e-06)


    optimizer = optim.SGD(model.parameters(), lr=lr, momentum=0.9, weight_decay=5e-4)
    scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[75, 90], gamma=0.1)  # 学习率衰减  # learning rate decay
    ...
    if attack_function == 'fgsm':
        ...
        adversary = attacks.LinfPGDAttack(model, eps=eps)
    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        ...
        if test_advacc > accuracy:
            #模型的名字
            model_name = f"{model_s}_{dataset}_{node_num}nodenum_{lr}lr_{img_size}size_200epoch_{seed}seed_{data_selection.type.split(' ')[1]}_normalization_SGD1.pth"
